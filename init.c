...
//		print("ServerType: %d\n", getServerType() );
        ...
        print("連接錯誤上限: %d\n", getAllowerrornum());
				...
				print("戰鬥經驗倍數: %d倍\n", getBattleexp() );
#endif
#ifdef _Set_Born_Player	//自定義出生轉數+等級+點數
				print("出生人物轉數: %d轉\n", getNewplayerBornPoint());
				print("出生人物等級: %d級\n", getNewplayerlv());
				print("出生人物點數: %d點\n", getNewplayertrans());
#endif
		...
		print("禁止傳送地圖: 地圖1:%d 地圖2:%d 地圖3:%d 地圖4:%d 地圖5:%d\n"
	          "              地圖1:%d 地圖2:%d 地圖3:%d 地圖4:%d 地圖5:%d\n",getUnlawwarpfloor(0),
																																					getUnlawwarpfloor(1),
																																					getUnlawwarpfloor(2),
																																					getUnlawwarpfloor(3),
																																					getUnlawwarpfloor(4),
    ...
    /*  夫弘奶件邰菲毛請允  */
   	{
			...
			char buff[50];
			sprintf(buff,"gogo6302-%s-%d",getAccountserverpasswd(),6333);
