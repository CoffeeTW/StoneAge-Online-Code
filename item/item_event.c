		...
		if( !strcmp( NewRides[i].arg, buf1))	{
			...
			if(CHAR_getInt( charaindex, CHAR_TRANSMIGRATION) < ridetrans){
				...
				if( getStringFromIndexWithDelim(itemarg,"|", 2, buf1, sizeof(buf1)) != FALSE ){
					sprintf( token, "必須%d轉人以上才能學習騎%s。",ridetrans,buf1);
					CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
					return;
				}
			}
...
//				sscanf( arg, "消 %d", &deltime);
				...
				CHAR_DelItem( charaindex, haveitemindex);
				sprintf( msg, "加快寵物蛋的消化速度 %s 分鐘。", deltime);
				CHAR_talkToCli( charaindex, -1, msg, CHAR_COLORYELLOW);
...
void ITEM_SetLoverUser( int charaindex, int toindex, int haveitemindex)
{
   ...
   if( CHAR_getInt( toindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
   {
   		 CHAR_talkToCli( charaindex, -1, "只能和玩家結婚哦", CHAR_COLORYELLOW);
       return;
   }
   			...
   			strcmp( CHAR_getChar( toindex, CHAR_LOVE), "YES"))//第二次判斷雙方是否已經結婚
   {
	    if( itemindex != -1 ){
            ...
            {    
		           ...
		           sprintf( token, "恭喜%s和%s新婚之喜，恭祝他們百年好合.白頭到老.", 
														           CHAR_getChar( charaindex, CHAR_NAME),
														           CHAR_getChar( toindex, CHAR_NAME));
		        ...
		        return;
						...
						if( emptyitemindexinchara < 0 ){
								...
								CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
								sprintf( token, "%s向您發起求婚，但您的物品欄已滿。" , CHAR_getChar( charaindex, CHAR_NAME));
								CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW);}
						else{
						 		CHAR_setItemIndex( toindex, emptyitemindexinchara, itemindex);
						   	sprintf( token, "%s向%s求婚的戒指!同意求婚請使用!", CHAR_getChar( charaindex, CHAR_NAME) 
																																							,CHAR_getChar( toindex, CHAR_NAME));
						...
						sprintf( token, "%s向您求婚，獲得對方的%s", CHAR_getChar( charaindex, CHAR_NAME),ITEM_getChar( itemindex, ITEM_NAME));
						CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW);
						sprintf( token, "已經向%s發起求婚", CHAR_getChar( toindex, CHAR_NAME));
						CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
						CHAR_DelItem( charaindex, haveitemindex);
				}
				CHAR_talkToCli( charaindex, -1, "請耐心等待對方回應！", CHAR_COLORYELLOW );    
			}else{
  ...
  if( !ITEM_CHECKINDEX(itemindex) ) return;
    ...
    if( checkUnlawWarpFloor( CHAR_getInt( i, CHAR_FLOOR) ) ) {
        ...
        if(strcmp(CHAR_getChar(i,CHAR_LOVERID),CHAR_getChar(charaindex,CHAR_CDKEY)) == 0 &&
          ...
          CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
          print("\n玩家%s 傳送到玩家%s身邊", CHAR_getChar( charaindex, CHAR_NAME),CHAR_getChar(i, CHAR_NAME));
          return;
        }
    }else
		    ...
		    if( itemindex != -1 ){
		   			...
		   			int emptyitemindexinchara = CHAR_findEmptyItemBox( toindex );
						...
						if( emptyitemindexinchara < 0 ){
								...
								CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
								sprintf( token, "%s向您發起離婚，但您的物品欄已滿。" , CHAR_getChar( charaindex, CHAR_NAME));
								CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW);}
					 	else{
			      	...
			      	sprintf( token, "%s向你申請離婚，如果同意請使用該戒指!", CHAR_getChar( charaindex, CHAR_NAME));
				      ITEM_setChar( itemindex, ITEM_EFFECTSTRING, token);
			      	CHAR_sendItemDataOne( i, emptyitemindexinchara);
			      	ITEM_endExistItemsOne(itemindex);
		       		sprintf( token, " %s向您提出離婚並把 %s 給你！", CHAR_getChar( charaindex, CHAR_NAME),ITEM_getChar( itemindex, ITEM_NAME));
		       		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW);
		       		sprintf( token, "已經向%s發起離婚", CHAR_getChar( i, CHAR_NAME));
		       		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
			      	CHAR_DelItem( charaindex, haveitemindex);
	          	return;}}
      }else{
...
void ITEM_ColorMetamo( int charaindex, int toindex, int haveitemindex)
{
		...
		/*{  造型 ,   紅  ,   綠  ,   黃  ,   灰  ,   白  ,   黑  },  //此行為說明行*/
			...
			{ 100180, 100190, 100195, 100185, 100180, 100790, 100795},	//短髮夾妹
		...
		if( strstr( itemarg, "紅" ) ){
				...
				switch( CHAR_getWorkInt( charaindex, CHAR_WORKFMFLOOR) ){
					...
					default:
						CHAR_talkToCli( charaindex, -1, "該戒指只能擁有莊園的光明家族成員使用！", CHAR_COLORYELLOW);
						return;
				}
#endif
				...
				switch( CHAR_getWorkInt( charaindex, CHAR_WORKFMFLOOR) ){
					...
					default:
						CHAR_talkToCli( charaindex, -1, "該戒指只能擁有莊園的黑暗家族成員使用！", CHAR_COLORYELLOW);
						return;
				}
#endif
...
void ITEM_CharaMetamo( int charaindex, int toindex, int haveitemindex)
{
		...
		if((OldMetamoId >= 100000 && OldMetamoId < 100240 )
			...
			if( strstr( itemarg, "豆丁囝" ) && OldMetamoId >= 100000 && OldMetamoId < 100120){
					...
					CHAR_talkToCli( charaindex, -1, "更改造型為大只佬", CHAR_COLORYELLOW);
			...
			}else if( strstr( itemarg, "短髮夾妹" ) && OldMetamoId >= 100120 && OldMetamoId < 100240){
					CHAR_talkToCli( charaindex, -1, "更改造型為短髮夾妹", CHAR_COLORYELLOW);
			...
			}else if( strstr( itemarg, "短髮夾妹" ) && OldMetamoId >= 100760 && OldMetamoId < 100820){
					CHAR_talkToCli( charaindex, -1, "更改造型為短髮夾妹", CHAR_COLORYELLOW);
								if( ( OldMetamoId - 100700 ) % 10 == 0 )
		...
		if(NewMetamoId==0){
			CHAR_talkToCli( charaindex, -1, "你的性別不同，無沒更改此造型!", CHAR_COLORYELLOW);
			return;}
		CHAR_setInt( charaindex , CHAR_BASEIMAGENUMBER , NewMetamoId );
		CHAR_setInt( charaindex , CHAR_BASEBASEIMAGENUMBER , NewMetamoId );
...
void ITEM_SexMetamo( int charaindex, int toindex, int haveitemindex)
{
		...
		if(OldMetamoId >=100000 && OldMetamoId <100020){                 //豆丁囝
				...
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成短髮夾妹!", CHAR_COLORYELLOW);
		...
		}else if(OldMetamoId>=100180 && OldMetamoId <100200){           //短髮夾妹
				NewMetamoId=OldMetamoId-160;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成賽亞人!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100200 && OldMetamoId <100220){           //手套女
				NewMetamoId=OldMetamoId-100;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成大只佬!", CHAR_COLORYELLOW);
				...
				NewMetamoId=OldMetamoId-60;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成辮子男孩!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100790 && OldMetamoId <100800){           //短髮夾妹
				NewMetamoId=OldMetamoId-80;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成賽亞人!", CHAR_COLORYELLOW);
...
void ITEM_AddMemberPoint( int charaindex, int toindex, int haveitemindex)
{
		...
		sprintf( token, "獲得會員點數%d,目前你擁有會員點數為%d!", atoi(itemarg),point+atoi(itemarg));
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );		
		CHAR_DelItem( charaindex, haveitemindex);
}
#endif
...
void ITEM_VipRide( int charaindex, int toindex, int haveitemindex)
{
		if(CHAR_getInt( charaindex , CHAR_VIPRIDE)!=1 
			...
			CHAR_talkToCli( charaindex, -1, "恭喜你，你已成為正式會員了！", CHAR_COLORYELLOW );		
			CHAR_DelItem( charaindex, haveitemindex);
		}else
			CHAR_talkToCli( charaindex, -1, "你已是會員了，沒必要再使用！", CHAR_COLORRED );
}
#endif
...
void ITEM_MetamoTime( int charaindex, int toindex, int haveitemindex )
{
	...
	if(metamoTime>60)
		...
		sprintf( msg, "變身成%s，可持續%d分鐘！", buff, metamoTime );
...
void ITEM_BattlePK( int charaindex, int toindex, int haveitemindex)
{
	...
	if( CHAR_getInt( toindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
	{
	   CHAR_talkToCli( charaindex, -1, "只能挑戰玩家哦！", CHAR_COLORYELLOW);
	   return;
	}
	if( charaindex == toindex )
	{
	...
	sprintf( token,"成功\向對方%s挑戰!",CHAR_getChar( toindex, CHAR_NAME));
	CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
	sprintf( token,"對方%s正式向你挑戰!",CHAR_getChar( charaindex, CHAR_NAME));
	CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW);
	CHAR_DelItem( charaindex, haveitemindex);
}
#endif
...
void ITEM_PetLevelItem( int charaindex, int toindex, int haveitemindex)
{
  ...
  if( CHAR_getInt( toindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
  {
  	CHAR_talkToCli( charaindex, -1, "該物品只能給寵物使用！", CHAR_COLORYELLOW);
    return;
  }
...
void ITEM_GetMultiItem( int charaindex, int toindex, int haveitemindex)
{
  ...
  itemnum = atoi(buf);
	...
	if( (itemnum - 1) <= CHAR_findSurplusItemBox( charaindex )){
		...
		sprintf( token, "盒子裡有%d件物品，你的物品欄位不足。", itemnum);
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
		return;
	}
