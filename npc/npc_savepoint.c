                     ...
                     int color )
{
	...
	if(NPC_CheckFlg(CHAR_getInt(talkerindex,CHAR_SAVEPOINT),oldmanid)==FALSE){
	    ...
	    // Robin 0619 display time
	    {
	        ...
	        ss=(int) new_t;
	        	...
	        	snprintf( timeMsg, sizeof( timeMsg ) ,
        			  "本星球%s已經啟動了 %d 日 %d 小時 %d 分 %d 秒。",getGameserverID(),dd,hh,mm,ss);
                } else if (hh>0) {
 	                snprintf( timeMsg, sizeof( timeMsg ) ,
      		                  "本星球%s已經啟動了 %d 小時 %d 分 %d 秒。",getGameserverID(),hh,mm,ss);
                } else {
                        snprintf( timeMsg, sizeof( timeMsg ) ,
                                  "本星球%s已經啟動了 %d 分 %d 秒。",getGameserverID(),mm,ss);
                }
                strcat(buf2,timeMsg);
#else
					if (dd>0) {
		...
		// Robin 03/20 萬能的天神，請賜給我灌水的力量!!!!
					...
					srand(time(0));
					if(getPlayerNum()>0)
						sprintf(temp,"目前線上人數為 %d 人。",abs(StateTable[LOGIN])+getPlayerNum()+rand()%10);
					else
