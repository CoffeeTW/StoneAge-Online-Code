			...
			case WINDOW_BUTTONTYPE_OK:
				{
					...
					if( CHAR_getInt( talker, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER &&
					  ...
					  if(check==TRUE){
							for (i=CHAR_getInt( talker, CHAR_LISTPAGE); i<CHAR_getInt( talker, CHAR_LISTPAGE)+5; i++){	// 10個莊園
							  getStringFromIndexWithDelim(fmpointlist.pointlistarray[i], "|", 5, fmindex, sizeof(fmindex));
						 	  if(atoi(fmindex)<=0 ){
						   		check=TRUE;
						    	break;
					  ...
					  if(check==TRUE){
					  	...
					  	windowtype = WINDOW_MESSAGETYPE_MESSAGE;
					   	strcpy(pointbuf,"        　    「家族據地列表」\n\n");
							
						}
					}else{
						strcpy(pointbuf,"        　    「家族據地列表」\n\n");
					}
					strcat(pointbuf," 「地　點」「東」「南」「狀　　態」\n");
					...
					if(atoi(fmindex)<=0)strcpy(name,"  未佔領");
					sprintf(pointbuf,"%s  薩姆吉爾  %3s   %3s   %s\n", pointbuf, x, y, name);
					...
					if(atoi(fmindex)<=0)strcpy(name,"  未佔領");
					sprintf(pointbuf,"%s  瑪麗娜斯  %3s   %3s   %s\n", pointbuf, x, y, name);
					...
					if(atoi(fmindex)<=0)strcpy(name,"  未佔領");
					sprintf(pointbuf,"%s  卡魯他那  %3s   %3s   %s\n", pointbuf, x, y, name);
			...
			case WINDOW_BUTTONTYPE_PREV:
				{
					...
					if( (pointlistindex + 5) > 5){
							...
							if(atoi(fmindex)<=0)strcpy(name,"  未佔領");
							sprintf(pointbuf,"%s  塔 爾 塔  %3s   %3s   %s\n", pointbuf, x, y, name);
							...
							if(atoi(fmindex)<=0)strcpy(name,"  未佔領");
							sprintf(pointbuf,"%s  弗 列 頓  %3s   %3s   %s\n", pointbuf, x, y, name);
							...
							if(atoi(fmindex)<=0)strcpy(name,"  未佔領");
							sprintf(pointbuf,"%s  亞 伊 歐  %3s   %3s   %s\n", pointbuf, x, y, name);
							...
							if(atoi(fmindex)<=0)strcpy(name,"  未佔領");
							sprintf(pointbuf,"%s  瑞爾亞斯  %3s   %3s   %s\n", pointbuf, x, y, name);
							buttontype |= WINDOW_BUTTONTYPE_PREV;
...
void ApplyFamilyPoint( int meindex, int toindex, int select)
{
    ...
    if(memberlist[tkfmindex].fmjoinnum<30)
		...
		if(atoi(fmindex)<=0 && check==0){
	  	...
	  	if(check==0)
					sprintf(buf, "該莊園據點已有人佔領了，請到該莊園的踢館管理員處申請踢館搶奪莊園吧！");
			else if(check==1)
					sprintf(buf, "很抱歉，申請莊園據點家族人必數需滿30人！");
			else if(check==2)
					sprintf(buf, "很抱歉，申請莊園據點家族聲望必需3000以上！");
			lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,	WINDOW_BUTTONTYPE_OK,
	    												0,	CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX),	buf);
	  }
}
