	...
	case NPC_AUTOPK_JOINPK:
		if(autopktime>2){
...
#endif
			{
				...
				sprintf(buf1,"玩家%s參加比賽！",CHAR_getChar( talkerindex, CHAR_NAME));
				sprintf(buf2,"玩家%s參加比賽,你還沒參加嗎？本次獎品豐富，你還等什麼？快來吧！",CHAR_getChar( talkerindex, CHAR_NAME));
		...
		if(select==1){
			if(autopktime>2){
				sprintf(token, "              ★亂舞格鬥ＰＫ賽管理員★\n\n"
				               "當前離比賽開始時間還剩%d分鐘，你是否要參加當前比賽嗎？？？",autopktime);
			...
			strcat(token, "轉生  等級    玩    家\n");
			AutoPk_PKNum(token);
			buttontype = WINDOW_BUTTONTYPE_CANCEL;
			windowno = NPC_AUTOPK_PKLIST;
		}else if(select==3){
			char buff[64];
			sprintf(token, "              ★亂舞格鬥ＰＫ賽獎品領取★\n");
			strcat(token, "「獲勝」「獎　　　　　　　品」\n");
...
BOOL AutoPk_GetHistoryStr( char *token, int page )
{
	...
	sprintf(token, "              ★亂舞格鬥ＰＫ賽歷史★\n"
	               "             第 %s 屆亂舞格鬥獲獎名單\n",hstime);
	strcat(token, "「名次」「殺人」「玩    家」\n");
	sprintf(buf,  "　冠軍%7d     %s\n"
								"　亞軍%7d     %s\n"
								"　季軍%7d     %s\n"
								"　優勝%7d     %s\n"
...
void AutoPk_PKSystemInfo()
{
	...
	if(autopktime>0){
		char buf1[64];
		char buf2[128];
		sprintf(buf1, "距離截止參加PK比賽時間還剩%d分鐘！",autopktime);
		if(awardpetid!=-1 && awarditemid==-1)
			sprintf(buf2,"距離截止參加PK比賽時間還剩%d分鐘，本次比賽獎品為:編號:%d,寵物:%s",autopktime-2,awardpetid,awardpet);
		else if(awardpetid==-1 && awarditemid!=-1)
			sprintf(buf2,"距離截止參加PK比賽時間還剩%d分鐘，本次比賽獎品為:編號:%d,物品:%s",autopktime-2,awarditemid,awarditem);
		else if(awardpetid!=-1 && awarditemid!=-1)
			sprintf(buf2,"距離截止參加PK比賽時間還剩%d分鐘，本次比賽獎品為:編號:%d,寵物:%s和編號:%d,物品:%s",autopktime-2,awardpetid,awardpet,awarditemid,awarditem);
		else
			sprintf(buf2, "距離正式PK比賽時間還剩%d分鐘！",autopktime);
		AutoPk_PKSystemTalk(buf1, buf2);
	}else if(autopktime==0){
...
void AutoPk_ChampionShipSet( char *cdkey, char *name, int winnum, int win )
{
	if(win==0){
		...
		if(winnum>ship[0].winnum){
			sprintf( token, "%s排名第一，殺人總數%d人！", name, winnum);
			...
			sprintf( token, "%s排名第二，殺人總數%d人！", name, winnum);
			AutoPk_PKSystemTalk(token,token);
			...
			sprintf( token, "%s排名第三，殺人總數%d人！", name, winnum);
			AutoPk_PKSystemTalk(token,token);
...
void AutoPk_GetChampionShip( void )
{
	...
	if(strlen(ship[0].cdkey)>0){
		sprintf( token, "恭喜%s獲得本次PK比賽冠軍，殺人總數%d人！", ship[0].name, ship[0].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
	if(strlen(ship[1].cdkey)>0){
		sprintf( token, "恭喜%s獲得本次PK比賽亞軍，殺人總數%d人！", ship[1].name, ship[1].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
	if(strlen(ship[2].cdkey)>0){
		sprintf( token, "恭喜%s獲得本次PK比賽季軍，殺人總數%d人！", ship[2].name, ship[2].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
	sprintf( token, "恭喜%s和%s獲得本次PK比賽優勝獎！", ship[3].name,ship[4].name);
	AutoPk_PKSystemTalk(token,token);
...
void NPC_AUTOPKHealerTalked( int meindex )
{
  if( CHAR_getInt( meindex , CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER )	return;
	...
	|| (CHAR_getWorkInt( meindex, CHAR_WORKPARTYMODE) == 2) )
	{
		...
		CHAR_talkToCli( meindex, -1,"恭喜你獲勝，體力已全部回復。",CHAR_COLORBLUE);
	}else{
		int i = 0;
		int otherindex;
		...
		for( i=0 ; i < CHAR_PARTYMAX ; i++)
		{
			...
			if(otherindex != -1){
				...
				CHAR_talkToCli( otherindex, -1,"恭喜你獲勝，體力已全部回復。",CHAR_COLORBLUE);
			}
		}
	}
}
