...
#define PETSKILL_CODE 500   //yang mark skillcode數量
...
#ifdef _Ip_Restrict //yang ADD限制IP登入
int getiprestrict( void );
#endif
#ifdef _Set_Born_Player          //yang ADD自定義出生轉數+等級+點數
int getNewplayerBornPoint( void );
...
#endif
#ifdef _Eggfeed //yang ADD 自定義餵蛋次數
	int getEggfeed( void );
#endif
#ifdef _Proexp //yang ADD 職業經驗倍率
	int getProexp( void );
#endif
#ifdef _RideTransLimit           //yang ADD 限制騎寵不得超過一轉
	int getRidets( void );
#endif	
#ifdef _Compel_Store             //yang ADD 強存開關
