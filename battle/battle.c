		...
		if(CHAR_getInt(BattleArray[battleindex].rivalindex, CHAR_FLOOR)==20000 &&
			...
			if(BattleArray[battleindex].type != BATTLE_TYPE_WATCH 
			  ...
			  if(CHAR_CHECKINDEX(winindex) != FALSE){
					...
					sprintf(token,"PK戰況廣播：%s 戰勝 —— %s 戰敗", winname, lostname);
				}else
					sprintf(token,"PK戰況廣播：%s 逃跑 —— %s 逃跑", winname, lostname);
...
static int BATTLE_Battling( int battleindex )
{
	...
	#ifdef _PigFix  			  //yang FIX 烏力化和黑烏力化
			if( pszOption == "\0")   
			    petrate = 100, pettime = 60, pigbbi = 100250;
            else
			{
			 getStringFromIndexWithDelim(pszOption," ",1,temp,sizeof(temp)) ;
			 petrate = atoi(temp);  //機率
			 getStringFromIndexWithDelim(pszOption," ",2,temp,sizeof(temp)) ;
			 pettime = atoi(temp);  //時間(秒)
			 getStringFromIndexWithDelim(pszOption," ",3,temp,sizeof(temp)) ;
			 pigbbi = atoi(temp);   //圖像編號
			}
	#else
			if( pszOption == "\0")
