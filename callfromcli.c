		...
		if( CHAR_getInt( charaindex, CHAR_LASTTALKELDER) >= 0 )
		{
			...
			if( flg == 1 ){//回紀錄點
				...
				if(CHAR_getInt(charaindex,CHAR_FLOOR)==20000 
			  	...
			  	sprintf(buf, "膽小鬼%s退出當前比賽！",CHAR_getChar(charaindex,CHAR_NAME));
    ...
    fdid = CONNECT_getFdid( fd );
  {
		...
		for( i=0; i<playernum; i++){
			...
			if( !strcmp( CHAR_getChar( i, CHAR_CDKEY), cdkey) ){
				CHAR_talkToCli(i, -1, "因重複登入而掉線!",CHAR_COLORYELLOW);
   ...
   CHAR_setMyPosition( fd_charaindex, x,y,TRUE);
	...
	if( request == 1 )
	{
		...
		if(	nowFloor == 31706
...
#ifdef _Join_Forbid  			//yang ADD 禁止加入團隊
			|| nowFloor == getjoinforbidnum(0) || nowFloor == getjoinforbidnum(1)
			|| nowFloor == getjoinforbidnum(2) || nowFloor == getjoinforbidnum(3)
			|| nowFloor == getjoinforbidnum(4) || nowFloor == getjoinforbidnum(5)
			|| nowFloor == getjoinforbidnum(6) || nowFloor == getjoinforbidnum(7)
			|| nowFloor == getjoinforbidnum(8) || nowFloor == getjoinforbidnum(9)
#endif
#ifdef _TIME_TICKET
			...
			CHAR_talkToCli( fd_charaindex, -1, "這裡是不能組隊的區域！", CHAR_COLORRED);
			lssproto_PR_send( fd, 1, 0);  //send
			return;
		}
	}
#endif
...
//				print("\n 玩家(%s)使用技能錯誤!", CHAR_getUseName( charaindex ) );
