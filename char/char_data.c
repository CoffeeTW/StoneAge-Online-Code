...
#endif
{
...
#ifdef _PET_2TRANS //yang mark 寵物成長限制
	if( tran == 0 ){
	#ifdef _Pettranslimit            //yang ADD 寵物轉生能力限制
	if( ans > getPettranslimit() )	
	   ans = getPettranslimit();  
	#else
		if( ans > 150 )	
			ans = 150;
	#endif
	...
	#ifdef _Pettranslimit            //yang ADD 寵物轉生能力限制
	if( ans > getPettranslimit()+50 )	
	   ans = getPettranslimit()+50 ; 
	#else
		if( ans > 200 )	
			ans = 200;
	#endif
...
void CHAR_PlayerRide( int charaindex )
{
		int i,j;
		int MetamoList[5][13]={
		/*{ 小矮子   賽亞人  辮子男孩  酷哥   熊皮男   大個    小矮妹  熊皮妹  帽子妹  短髮夾妹  手套女   辣妹    虎}, 此行為說明行 */
			{ 100000, 100025, 100055, 100060, 100095, 100100, 100135, 100145, 100165, 100190, 100200, 100230, 309},	//紅
			{ 100005, 100030, 100050, 100065, 100085, 100115, 100120, 100140, 100170, 100195, 100210, 100225, 307},	//綠
			{ 100010, 100035, 100045, 100070, 100090, 100110, 100125, 100150, 100160, 100185, 100215, 100220, 310},	//黃
