...
void FAMILY_Join(int fd, int meindex, char *message)
{
   ...
   if (CheckFMMember(meindex) < 0){
	...
	if( CHAR_getInt( meindex, CHAR_FMTIMELIMIT ) > (int)time(NULL) ){
		char buff[255];
		sprintf(buff, "\n如之前退出家族，\n需滿%d小時才能再加入家族喔！",(CHAR_getInt( meindex, CHAR_FMTIMELIMIT )-(int)time(NULL))/3600+1);
		lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
...
#endif
#ifdef _VIP_SERVER
		sprintf(sendbuf,"你目前的個人會員點數為：%d",CHAR_getInt(meindex,CHAR_AMPOINT));
		CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORYELLOW);
#endif
	}
...
//shen_test
//printf("****** Shen *******\n%d  %d\n*************\n", fmdptop.fmMomentum[h], fmdptop.fmMomentum[h]/100);
...
int FAMILY_RidePet( int fd, int meindex, char* message )
{
	...
	if( strcmp( token, "P") == 0) {
		...
		if( atoi(token2) != -1 ) {
			...
			{ 
				char buff[255];
				sprintf(buff,"你目前只能騎乘等級小於%d級的寵。",CHAR_getInt( meindex, CHAR_LEARNRIDE));
			...
			{ 
				char buff[255];
				sprintf(buff,"你最高只能騎寵等級比你大%d級的寵。",getRideLevel());
				CHAR_talkToCli( meindex, -1, buff, CHAR_COLORYELLOW );
				return 0;
			}	
#else
...
#endif
#ifdef _RideTransLimit  //yang ADD 限制騎寵不得超過一轉
			if( CHAR_getInt( petindex, CHAR_TRANSMIGRATION) > 1 && !getRidets()) 
			{
				CHAR_talkToCli( meindex, -1, "不得騎超過一轉的騎寵唷~~", CHAR_COLORYELLOW );
				return 0;
			}
#endif
...
//					printf("\n******** Shen index ******\n%d\n***************\n",index);
		...
		//printf("**Before return 0 *****Shen Debug*******\n%d\n********************\n",rideGraNo);
