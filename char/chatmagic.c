...
void CHAR_CHAT_DEBUG_setmp( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		if (i>=playernum)
		{
		 	CHAR_talkToCli( charindex, -1, "此帳號不在線~", CHAR_COLORYELLOW );   
			return;
		}
		...
		sprintf( token, "玩家%s的MP的設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(setmp)/100);
...
void CHAR_CHAT_DEBUG_str( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的腕力設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(str)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_tgh( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的耐力設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(tgh)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_dex( int charindex , char *message )
{
  ...
  if(strlen(cdkey)>0){
	  ...
	  if (i>=playernum)
	  {
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的敏捷設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(dex)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_vital( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的體力設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(vital)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_luck( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的運氣設置為%d!", CHAR_getChar( i, CHAR_NAME),luck);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_gold( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的金錢設置為%d!", CHAR_getChar( i, CHAR_NAME),money);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
       ...
       return;
		...
		for( i = 0; i < num; i ++ ) {
			...
			if( emptyitemindexinchara < 0 ){
				sprintf( msgbuf, "玩家%s物品欄位不足。", CHAR_getChar( j, CHAR_NAME ));
		...
		atoi( message),
				...
				CHAR_talkToCli( j, -1,msgbuf, CHAR_COLORYELLOW);
				snprintf( msgbuf, sizeof( msgbuf), "成功\為%s製作 %s。",
	    ...
	    if (i>=playernum)
	     {
			...
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "玩家%s的人物形象設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(metamoid));
			CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
    }else{
...
void CHAR_CHAT_DEBUG_announce(int charindex , char *message )
{
  ...
  for( i = 0 ; i < playernum ; i++) {
    if( CHAR_getCharUse(i) != FALSE ) {
       char token[256];
			 sprintf(token, "<服務器公告> %s 說：%s", CHAR_getChar( charindex , CHAR_NAME), message);
...
void CHAR_CHAT_DEBUG_level( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		if (i>=playernum)
		{
		 	CHAR_talkToCli( charaindex, -1, "此帳號不在線~", CHAR_COLORYELLOW );
			return;
		}
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的等級設置為%d!", CHAR_getChar( i, CHAR_NAME),level);
...
void CHAR_CHAT_DEBUG_exp( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的經驗設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(exp));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_GetMap( int charaindex, char* message ){
  ...
  int		tile, obj;
	...
	{	
		char szBuffer[64];
		snprintf( szBuffer, sizeof( szBuffer ), "圖片= %d; 對像= %d",
			tile, obj );
...
void CHAR_CHAT_Map( int charaindex, char* message ){
  ...
  char  buf[16];
	...
	{	
		...
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
		snprintf( szBuffer, sizeof( szBuffer ), "對像：%d => %d",	obj, fixobj );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
	}
...
void CHAR_CHAT_Fixtile( int charaindex, char* message ){
  ...
  int		fixtile=atoi(message);
	...
	{	
		...
		snprintf( szBuffer, sizeof( szBuffer ), "圖片：%d => %d",	tile, fixtile );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
		snprintf( szBuffer, sizeof( szBuffer ), "對像：%d => %d",	obj, obj );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
	}
...
#define		CHAT_CHAT_DEBUG_SUPERMAN_PARAMDEF		100000
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s設置為超人狀態!", CHAR_getChar( i, CHAR_NAME));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}else{
		CHAR_setInt(charaindex,CHAR_SKILLUPPOINT, 0);
		CHAR_Skillupsend(charaindex);
...
void CHAR_CHAT_DEBUG_petmake( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "給玩家%s的%s製作成功\!", CHAR_getChar( j, CHAR_NAME),ENEMY_getChar( i, ENEMY_NAME));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
...
{	
#ifdef _512_Event                //yang ADD 多增任務旗標至512
	int event_num = 16;
#else
...
void CHAR_CHAT_DEBUG_dp( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的DP設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(dp));
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_setTrans( int charaindex, char *message)
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s轉生設置為%d轉!", CHAR_getChar( i, CHAR_NAME),(int)atoi(setTrans));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_petlevelup( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		if( !CHAR_CHECKINDEX( petindex) ){
			sprintf( token, "玩家%s寵物欄%d上沒寵物!", CHAR_getChar( i, CHAR_NAME),petid+1);
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			return;
		}
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的寵物欄%d的寵物設為%d級!", CHAR_getChar( i, CHAR_NAME), 
																										petid+1, CHAR_getInt( petindex, CHAR_LV));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
...
#ifdef _GM_KING
                snprintf( line, sizeof( line),
               		"%s大魔王將 %s 封嘴%d分鐘。",CHAR_getChar(index,CHAR_NAME),
                	CHAR_getChar( index, CHAR_NAME),
                	minu );
#else
                snprintf( line, sizeof( line),
...
void CHAR_CHAT_DEBUG_getyou(int charaindex,char *message)
{
	...
	area = atoi(areabuf);
	if( area <= 0 || area > 3 ){
        sprintf(buf,"%s","沒有輸入  圍或  圍超出限制，請輸入1~3");
        CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
        return;
	}
...
void CHAR_CHAT_DEBUG_fsize( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩家%s的字體設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(fsize));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_MM( int charaindex , char *message )
{
	...
	if(index==1)enemyid=1479;
		...
		CHAR_talkToCli( charaindex, -1, "目前只能製作1/2代瑪蕾菲雅!", CHAR_COLORYELLOW );
		return;}
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( charaindex, -1, token,  CHAR_COLORWHITE);
		sprintf( token, "[GM]%s製作了只極品%d代瑪蕾菲雅給你!", CHAR_getChar( charaindex, CHAR_NAME),index);
		CHAR_talkToCli( j, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "給玩家%s製作了只極品%d代瑪蕾菲雅!", CHAR_getChar( j, CHAR_NAME),index);
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		for( i = 0; i < CHAR_MAXPETHAVE; i ++ ){
			if( CHAR_getCharPet( j, i ) == ret )break;
		}
...
void CHAR_CHAT_DEBUG_Sendto( int charaindex , char *message )
{
    ...
    if(!strcmp(CHAR_getChar(i,CHAR_CDKEY),cdkey))
    {
       ...
       sprintf( token, "已傳送到玩家%s身邊！", CHAR_getChar( i, CHAR_NAME));
       CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
       return;
     }
   }
...
void CHAR_CHAT_DEBUG_GMFUNCTION( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "給玩家%s使用%s權限%d次!", CHAR_getChar( i, CHAR_NAME), gmfunction, atoi( gmtime ));
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_SETRIDE( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "給玩家%s騎乘騎寵%s的資格!", CHAR_getChar( i, CHAR_NAME), NewRides[rideindex-1].name);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_MVRIDE( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "移除玩家%s騎乘騎寵%s的資格!!", CHAR_getChar( i, CHAR_NAME), NewRides[rideindex-1].name);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		int LRCode = CHAR_getInt( charindex, CHAR_LOWRIDEPETS);
		LRCode = LRCode^NewRides[rideindex-1].Code;
...
void CHAR_CHAT_DEBUG_SHOWIP( int charindex , char *message )
{
	...
	if(strlen(message)>0){
	  ...
	  d=(ip % 0x100);
		sprintf( token, "帳號%s的IP是 %d.%d.%d.%d",message, a,b,c,d);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
#endif
...
{
	char szMsg[256];
	sprintf(szMsg,"已設置PK時間為%d分鐘!",atoi(message));
	CHAR_talkToCli(charaindex,-1,szMsg,CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_Trans( int charaindex, char *message)
{
	...
	if(strlen(cdkey)>0){
		...
		if(Trans>7 || CHAR_getInt(i,CHAR_LV)<80){
			...
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "由於玩家%s已七轉或等級小於80級，所以%d轉生失敗!", CHAR_getChar( i, CHAR_NAME),Trans);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
			return;
		}else{
			CHAR_sendCToArroundCharacter( CHAR_getWorkInt( i, CHAR_WORKOBJINDEX ));
			...
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "為玩家%s轉生為%d轉!", CHAR_getChar( i, CHAR_NAME),Trans);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
			charaindex=i;
		}
	}else{
		...
		if(Trans>7 || CHAR_getInt(charaindex,CHAR_LV)<80){
			sprintf( token, "由於你已七轉或等級小於80級，所以轉生失敗!", CHAR_getChar( charaindex, CHAR_NAME));
			CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
			return;
		}else{
			CHAR_sendCToArroundCharacter( CHAR_getWorkInt( charaindex, CHAR_WORKOBJINDEX ));
...
void CHAR_CHAT_DEBUG_fusionbeit( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s為你身上的第%d只寵去掉融合寵限制!", CHAR_getChar( charaindex, CHAR_NAME),petid+1);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "為玩家%s身上的第%d只寵去掉融合寵限制!", CHAR_getChar( i, CHAR_NAME),petid+1);
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		snprintf( token, sizeof( token ), "K%d", petid );
		CHAR_sendStatusString( i, token );
		snprintf( token, sizeof( token ), "W%d", petid );
		...
		CHAR_setInt( petindex, CHAR_FUSIONBEIT ,0);	
		sprintf( token, "將身上的第%d只寵去掉融合寵限制!", petid+1);
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		CHAR_complianceParameter( petindex );
		snprintf( token, sizeof( token ), "K%d", petid );
		CHAR_sendStatusString( charaindex, token );
