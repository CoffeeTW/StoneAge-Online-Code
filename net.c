...
typedef struct tagCONNECT
{
  ...
  int charaindex;     /* char?昫尺及奶件犯永弁旦﹝
                               * 夫弘奶件詨卞襖互澀爛今木月﹝-1互犯白巧伙玄
                               * ?昫卞卅中凜﹝
                               */
  char CAbuf[ 2048 ];         /*  CA() 毛做諒允月啃及田永白央 */
  int CAbufsiz;       /*  CAbuf 及扔奶術  */

  struct timeval lastCAsendtime;     /*瘉詨卞CA毛霜勻凶凜棉 */

  char CDbuf[ 2048 ];         /*  CD() 毛做諒允月啃及田永白央 */
  int CDbufsiz;       /*  CDbuf 及扔奶術  */

  struct timeval lastCDsendtime;     /*瘉詨卞CD毛霜勻凶凜棉 */

  struct timeval lastCharSaveTime; /* 瘉詨卞平乓仿犯□正毛本□皮仄凶凜棉 */

  struct timeval lastprocesstime;    /* 瘉詨卞皿夫玄戊伙毛質咥仄凶凜棉*/

  struct timeval lastreadtime;       /* 瘉詨卞read仄凶凜棉﹝曉午反切互丹*/
...
CONNECT *Connect;     /*ヵбヱЁъ⑦瘦グ脫*/


/* 簇眶ソ黎片ズコんサマろペプよズエペクんソсヱ① */
#define SINGLETHREAD
#define MUTLITHREAD
#define ANYTHREAD
...
/*------------------------------------------------------------
 * fdid ろヘワцьソindex メパシバペ。
 * 苞眶
 *  fdid    int     fdソid
 * 手ベ猛
 *  -1 ソ箕ゾ①ヲユ⑦面ソワцьゾノコろヘスろゲギ。0笆懼スヘ
 * ①ヲユ⑦面ソワцьソワцьゾゆホコデソ index
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * ワцьindex ろヘ fdid メパシバペ。
 * 苞眶
 *  charind  int     иャユюЫュЗヱэкУ
 * ろりベグ fdid  砷クゲギヘワцьindゎれろウゆ
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * fdズ充ベ碰サヘホギ儡魯ゎヤロヨ⑦ЬД【дクシ、BOOLメ手エ
 * 嘿ゆリь【ХラЧヱゾウスゆ。
 * 苞眶
 *  fd  int     иャユюЫュЗヱэкУ
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * 鏈鎊ソ儡魯メ磊ゲサ、ヤロヨ⑦ЬД【дズЫ【Уメ瘦賂ウプよシエペ。
 * 苞眶、手ベ猛
 *  スウ
 ------------------------------------------------------------*/
...
*/
  ...
  if( ret > 0 && FD_ISSET(bindedfd , &rfds ) ){
    ...
    int sockfd;
//    int flags=fcntl(bindedfd,F_GETFL,0);//通過 F_GETFL 獲取文件描述標誌給flags
//		fcntl(bindedfd,F_SETFL,flags|O_NONBLOCK|O_NDELAY);//此處只是把描述符寄存位制為O_NONBLOCK
    ...
    if( sockfd == -1 && errno == EINTR ){
      ...
      if(isThereThisIP( sinip) ){ //yang ADD 限制IP登入
				// Nuke +2 Errormessage
				char mess[64]="E你的IP登入數量已達上限，請勿在登入！";
				if (!from_acsv)
      ...
      }else if (strcmp(Connect[ sockfd ].cdkey, "yangserver")!=0 || strcmp(Connect[ sockfd ].cdkey, "")!=0) {
        // Nuke +2 Errormessage	            
...
/*
							sprintf( buff1,"\n玩家=%d 寵物=%3.1f%% 物品=%3.1f%% 郵件:%d 戰鬥:%d\n%s",
...
//      int flags=fcntl(fdremember,F_GETFL,0);//通過 F_GETFL 獲取文件描述標誌給flags
//			fcntl(fdremember,F_SETFL,flags|O_NONBLOCK|O_NDELAY);//此處只是把描述符寄存位制為O_NONBLOCK
...
//By Shen 暫時修正訊號錯誤
#ifdef _FIX_NU
  if ( Connect[ fd ].nu < 0 ) Connect[fd].nu = 20;
#else
...
//	BOOL bReuseaddr=TRUE;
	...
	rLinger.l_onoff  = 1;   // 打開linegr開關
  rLinger.l_linger = 0;   // 設置延遲時間為 0 秒, 注意 TCPIP立即關閉，但是有可能出現化身
...
//	int flags=fcntl(sockfd,F_GETFL,0);//通過 F_GETFL 獲取文件描述標誌給flags
//	fcntl(sockfd,F_SETFL,flags|O_NONBLOCK|O_NDELAY);//此處只是把描述符寄存位制為O_NONBLOCK
}
